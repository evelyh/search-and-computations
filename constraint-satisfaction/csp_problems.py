...
def sudokuCSP(initial_sudoku_board, model='neq'):
    ...
    for row in var_array:
        if model == 'neq':
            ...
            constraint_list.append(
                AllDiffConstraint("Row [{}]".format(row), row))
    ...
    for colj in range(len(var_array[0])):
        ...
        if model == 'neq':
            ...
            constraint_list.append(
                AllDiffConstraint("Column [{}]".format(colj), scope))
    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
                ...
                constraint_list.append(
                    AllDiffConstraint("Subsquare [{},{}]".format(i, j), scope))
    ...
    def __init__(self, planes, flights, can_fly, flights_at_start,
        ...
        for l in flights_at_start:
            for f in l[1:]:
                if f not in flights:
                    print(
                        "PlaneProblem Error, flights_at_start contains a non-flight",
                        f)
...
def solve_planes(planes_problem, algo, allsolns,
    ...
    def planeScheduling(problems):
        """CSP for plane scheduling"""
        variables = []
        planes = problems.planes[:]
        can_fly = problems._can_fly
        for plane in range(len(planes)):
            variables.append([])
            domain = [0] + can_fly[planes[plane]]
            for seq in range(len(can_fly[planes[plane]])):
                variable = Variable("{},{}".format(planes[plane], seq), domain)
                variables[plane].append(variable)

        constraints = []
        # Constraint1: start
        start = problems._flights_at_start
        for i in range(len(variables)):
            vals = [[0]]
            for st in start[planes[i]]:
                vals.append([st])
            first = variables[i][0]
            constraint = TableConstraint("Valid Starts [{}]".format(first), [first], vals)
            constraints.append(constraint)

        # Constraint2: can_follow
        flights = problems.flights[:]
        follow = problems.can_follow[:]
        connections = [[0, 0]]
        for flight in flights:
            connections.append([flight, 0])
        for item in follow:
            connections.append(list(item))
        for i in range(len(variables)):
            for j in range(len(variables[i]) - 1):
                first = variables[i][j]
                last = variables[i][j + 1]
                constraint = TableConstraint(
                    "Valid Connections [{}]".format(variables[i]), [first, last], connections)
                constraints.append(constraint)

        # Constraint3: maintenance
        maintenance = problems.maintenance_flights[:]
        freq = problems.min_maintenance_frequency
        required = [0] + maintenance
        for i in range(len(variables)):
            for j in range(len(variables[i]) + 1 - freq):
                constraint = NValuesConstraint(
                    "Valid Maintenance [{}]".format(variables[i]), variables[i][j:j + freq], required, 1, freq)
                constraints.append(constraint)

        # Constraint4: no_repeat
        vars = []
        for h in variables:
            for g in h:
                vars.append(g)
        constraint = NoRepeatConstraint("no_repeat", vars, flights)
        constraints.append(constraint)

        return CSP("PlaneScheduling", vars, constraints)

    # invoke search with the passed parameters
    csp = planeScheduling(planes_problem)
    solutions, nodesExplored = bt_search(algo, csp, variableHeuristic, allsolns, trace)
    ...
    results = []
    if (len(solutions) == 0) and (not silent):
        print("No solutions to {} found".format(csp.name()))
        ...
        for x in solutions:
            ...
            for (var, val) in x:
                ...
                sequence = var.name().split(',')[1]
                if name in temp_dict:
                    temp_dict[name].append([sequence, val])
                else:
                    temp_dict[name] = [[sequence, val]]
            lst = []
