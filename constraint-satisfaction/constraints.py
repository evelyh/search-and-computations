...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        self.j = j
        scope = [qi, qj]
        satAssi = self.create_satAssi(qi, qj)
        TableConstraint.__init__(self, name, scope, satAssi)
        self._name = "Queen_" + name

    def create_satAssi(self, qi, qj):
        dom = []
        for i in qi.domain():
            dom.append(i)
        satAssi = []
        diag = abs(self.i - self.j)
        for j in dom:
            for h in dom:
                if (j != h) and (abs(j-h) != diag):
                    satAssi.append([j, h])
        return satAssi
...
class NValuesConstraint(Constraint):
    ...
    def hasSupport(self, var, val):
        ...
        def bounds_checker(assi):
            '''helper function to check both bounds'''
            total = 0
            for (var, val) in assi:
        ...
        def ub_checker(assi):
            '''helper function to check upper bounds'''
            total = 0
            for (var, val) in assi:
                if val in self._required:
                    total += 1
        ...
        remain = self.scope()
        remain.remove(var)
        return findvals(remain, [(var, val)], bounds_checker, ub_checker)


class NoRepeatConstraint(Constraint):
    def __init__(self, name, scope, values):
        Constraint.__init__(self,name, scope)
        self._name = "NoRepeat_" + name
        self._scope = scope
        self._values = values
    ...
    def check(self):
        ...
        for var in self.scope():
            if not var.isAssigned():
                ...
                flight = var.getValue()
                if flight != 0:
                    if flight in freq_dict:
                        freq_dict[flight] += 1
                    else:
                        freq_dict[flight] = 1
        if len(freq_dict) < len(set(self._values)):
            return False
        for flight in freq_dict:
            if freq_dict[flight] > 1:
                return False
        return True
    ...
    def hasSupport(self, var, val):
        ...
        def extension(assi):
            return len(set(self._values)) <= len(set(self._scope))

        remain = self.scope()
        remain.remove(var)
        return findvals(remain, [(var, val)], no_repeat, extension)
