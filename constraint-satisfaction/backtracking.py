...
def FC(unAssignedVars, csp, allSolutions, trace):
    ...
    if unAssignedVars.empty():
        for i in csp.variables():
            results.append((i, i.getValue()))
        return [results]

    var = unAssignedVars.extract()
    bt_search.nodesExplored += 1
    for j in var.curDomain():
        var.setValue(j)
        noDWO = True
        for h in csp.constraintsOf(var):
            if h.numUnassigned() == 1:
                if FCCheck(h, var, j) == "DWO":
                    noDWO = False
                    break
        if noDWO:
            extension = FC(unAssignedVars, csp, allSolutions, trace)
            if extension:
                results.extend(extension)
            if (not allSolutions) and (len(results) > 0):
                Variable.restoreValues(var, j)
                break
        Variable.restoreValues(var, j)
    var.unAssign()
        ...
        for var in c.scope():
            for val in var.curDomain():
                if not c.hasSupport(var, val):
                    ...
                    if var.curDomainSize() == 0:
                        return "DWO"
                    for cc in csp.constraintsOf(var):
                        if (cc not in constraints) and (cc is not c):
...
def GAC(unAssignedVars, csp, allSolutions, trace):
    ...
    if unAssignedVars.empty():
        ...
        for var in csp.variables():
            results.append((var, var.getValue()))
        return [results]
    ...
    for val in var.curDomain():
        ...
        if GacEnforce(csp.constraintsOf(var), csp, var, val) == "DWO":
            noDWO = False
        if noDWO:
            extension = GAC(unAssignedVars, csp, allSolutions, trace)
            if extension:
                all_results.extend(extension)
            if (not allSolutions) and (len(all_results) > 0):
